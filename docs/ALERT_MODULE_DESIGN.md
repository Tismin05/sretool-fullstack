...
func (s *EmailSender) buildSubject(alerts []Alert) string {
    ...
    if errorCount > 0 {
        return fmt.Sprintf("[%s] %d Error Alerts from sretool-fullstack", m.Host, errorCount)
    }
    return fmt.Sprintf("[%s] %d Warning Alerts from sretool-fullstack", m.Host, warnCount)
}
...
func (s *EmailSender) buildBody(alerts []Alert) string {
    ...
    buf.WriteString(strings.Repeat("=", 50))
    buf.WriteString("\n")
    buf.WriteString("Generated by sretool-fullstack\n")
